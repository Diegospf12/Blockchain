//
// Created by Diego Pacheco Ferrel on 21/05/23.
//

#ifndef BLOCKCHAIN_B_TREE_H
#define BLOCKCHAIN_B_TREE_H
...
public:
    NodeBPlus(std::size_t _degree) {// Constructor
        this->is_leaf = false;
        this->degree = _degree;
        this->size = 0;
...
public:
    ...
    NodeBPlus<T>* BPlusTreeSearch(NodeBPlus<T>* node, T key){
        if(node == nullptr) { // if root is null, return nullptr
            return nullptr;
        }
        else{
            NodeBPlus<T>* cursor = node; // cursor finding key
    ...
    NodeBPlus<T>* BPlusTreeRangeSearch(NodeBPlus<T>* node, T key){
        if(node == nullptr) { // if root is null, return nullptr
            return nullptr;
        }
        else{
    ...
    std::vector<T> range_search(T start, T end) {
        std::vector<T> result_data;
        NodeBPlus<T>* start_node = BPlusTreeRangeSearch(this->root, start);
